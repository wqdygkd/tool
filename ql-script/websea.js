...
// ==================================异步顺序==============================================================================
await main().catch(error => $.logErr(error))
  .finally(() => $.done())
//= =================================脚本入口函数main()==============================================================
  ...
  try {
    ...
    if (data.errno === 0) {
      ...
      if (result?.traderOperation) {
        const list = traderOperation.filter(item => {
          return item.time >= result.traderOperation[0].time
        })
        let message_ = ''
        for (const item of list) {
          message_ += `时间 ${dayjs(item.time * 1000).format('YYYY-MM-DD HH:mm:ss')}  合约 ${buyOrSell[item.buyOrSell]} ${item.symbol} 数量${item.amountConvert} 价格 ${item.price} 盈亏 ${item.profitLoss}\n`
        }
        if (message_) {
...
function httpRequest (options, timeout = 1 * 1000) {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      $[method](options, (error, resp, data) => {
        try {
          if (error) {
  ...
  try {
    ...
    for (const url of urls) {
      ...
      if (result && 'notice' in result) {
        notice = result.notice.replaceAll('\\n', '\n')
        break
      }
    }
