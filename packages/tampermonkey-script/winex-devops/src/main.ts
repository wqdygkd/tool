...
if (location.pathname === '/cluster/app/win/webclient') {
  setInterval(() => {
    try {
      let U = window.pageUI
      U.getViewPart('main').getComponent('toolbar').getButton('toolbar_toolbaritem_web').setActive(true)
      U.getViewPart('main').getComponent('toolbar').getButton('toolbar_toolbaritem_patch').setActive(true)
    } catch {}
  }, 1000)
}
